defmodule MusicListings.Parsing.DakotaTavernParserTest do
  use ExUnit.Case, async: true

  alias MusicListings.Parsing.DakotaTavernParser
  ...
  setup do
    ...
    event =
      single_event_file_path
      |> File.read!()
      |> DakotaTavernParser.event_selector()
      |> List.first()
  ...
  describe "venue_name/0" do
    test "returns expected value" do
      assert "The Dakota Tavern" == DakotaTavernParser.venue_name()
    end
  end

  describe "event_selector/1" do
    test "returns expected events", %{index_html: index_html} do
      events = DakotaTavernParser.event_selector(index_html)
  ...
  describe "age_restriction/1" do
    test "returns the event age restriction", %{event: event} do
      assert :tbd == DakotaTavernParser.age_restriction(event)
    end
  end
