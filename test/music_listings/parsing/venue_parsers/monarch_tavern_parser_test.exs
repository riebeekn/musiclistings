defmodule MusicListings.Parsing.VenueParsers.MonarchTavernParserTest do
  ...
  describe "source_url/0" do
    test "returns expected value" do
      assert String.starts_with?(
               ...
               "https://tockify.com/api/ngevent?max=200&view=agenda&calname=monarchtavern&start-inclusive=true&longForm=false&showAll=false&startms="
             )
    end
  end
  ...
  describe "additional_dates/1" do
    test "returns a list of additional dates", %{event: event} do
      assert [] == MonarchTavernParser.additional_dates(event)
    end
  end
