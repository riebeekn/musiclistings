defmodule MusicListings.Parsing.VenueParsers.RoyThomsonHallParserTest do
  ...
  describe "next_page_url/2" do
    test "returns the next page url", %{index_html: index_html} do
      assert nil == RoyThomsonHallParser.next_page_url(index_html, nil)
    end
  end
  ...
  describe "ignored_event_id/1" do
    test "returns ignored event id", %{event: event} do
      assert "5958" == RoyThomsonHallParser.ignored_event_id(event)
    end
  end
  ...
  describe "additional_dates/1" do
    test "returns a list of additional dates", %{event: event} do
      assert [] == RoyThomsonHallParser.additional_dates(event)
    end
  end
  ...
  describe "details_url/1" do
    test "returns the event details url", %{event: event} do
      assert "https://roythomsonhall.mhrth.com/tickets/final-fantasy-vii-rebirth-orchestra-world-tour/" ==
