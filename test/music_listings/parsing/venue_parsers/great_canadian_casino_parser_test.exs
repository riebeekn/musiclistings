defmodule MusicListings.Parsing.VenueParsers.GreatCanadianCasinoParserTest do
  ...
  describe "event_title/1" do
    test "returns event title", %{event: event} do
      assert "Trivium & Bullet for My Valentine: The Poisoned Ascendancy" ==
  ...
  describe "performers/1" do
    test "returns the event performers", %{event: event} do
      assert %Performers{
               headliner: "Trivium",
               openers: ["Bullet for My Valentine", "August Burns Red", "Bleed From Within"]
  ...
  describe "event_date/1" do
    test "returns the event date", %{event: event} do
      assert ~D[2025-05-04] == GreatCanadianCasinoParser.event_date(event)
    end
  end

  describe "additional_dates/1" do
    test "returns a list of additional dates", %{event: event} do
      assert [] == GreatCanadianCasinoParser.additional_dates(event)
    end
  end

  describe "event_time/1" do
    test "returns the event start time", %{event: event} do
      assert ~T[18:30:00] == GreatCanadianCasinoParser.event_time(event)
    end
  end
  ...
  describe "ticket_url/1" do
    test "returns the event ticket url", %{event: event} do
      assert "https://www.ticketmaster.ca/trivium-bullet-for-my-valentine-the-toronto-ontario-05-04-2025/event/1000612DB5852BBE" ==
