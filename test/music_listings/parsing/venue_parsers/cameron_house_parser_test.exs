defmodule MusicListings.Parsing.VenueParsers.CameronHouseParserTest do
  ...
  describe "events/1" do
    test "returns expected events", %{index_html: index_html} do
      events = CameronHouseParser.events(index_html)
  ...
  describe "next_page_url/2" do
    test "returns the next page url", %{index_html: index_html} do
      assert nil == CameronHouseParser.next_page_url(index_html, nil)
    end
  end
  ...
  describe "additional_dates/1" do
    test "returns a list of additional dates", %{event: event} do
      assert [] == CameronHouseParser.additional_dates(event)
    end
  end
  ...
  describe "age_restriction/1" do
    test "returns the event age restriction", %{event: event} do
      assert :unknown == CameronHouseParser.age_restriction(event)
    end
  end
