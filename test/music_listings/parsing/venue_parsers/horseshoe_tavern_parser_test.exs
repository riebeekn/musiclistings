defmodule MusicListings.Parsing.VenueParsers.HorseshoeTavernParserTest do
  ...
  describe "next_page_url/2" do
    test "returns the next page url", %{index_html: index_html} do
      assert nil == HorseshoeTavernParser.next_page_url(index_html, nil)
    end
  end

  describe "event_id/1" do
    test "returns event id", %{event: event} do
      assert "horseshoe_tavern_2024_09_06_" == HorseshoeTavernParser.event_id(event)
    end
  end

  describe "ignored_event_id/1" do
    test "returns ignored event id", %{event: event} do
      assert "horseshoe_tavern_2024_09_06_" == HorseshoeTavernParser.ignored_event_id(event)
    end
  end
