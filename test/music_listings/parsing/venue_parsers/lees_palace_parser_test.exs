defmodule MusicListings.Parsing.VenueParsers.LeesPalaceParserTest do
  ...
  describe "events/1" do
    test "returns expected events", %{index_html: index_html} do
      events = LeesPalaceParser.events(index_html)
  ...
  describe "next_page_url/2" do
    test "returns the next page url", %{index_html: index_html} do
      assert nil == LeesPalaceParser.next_page_url(index_html, nil)
    end
  end

  describe "event_id/1" do
    test "returns event id", %{event: event} do
      assert "whitehall_2024_07_27" == LeesPalaceParser.event_id(event)
    end
  end

  describe "ignored_event_id/1" do
    test "returns ignored event id", %{event: event} do
      assert "whitehall_2024_07_27" ==
  ...
  describe "additional_dates/1" do
    test "returns a list of additional dates", %{event: event} do
      assert [] == LeesPalaceParser.additional_dates(event)
    end
  end
