defmodule MusicListings.Parsing.ConcertHallParserTest do
  use ExUnit.Case, async: true

  alias MusicListings.Parsing.ConcertHallParser
  ...
  setup do
    ...
    event =
      single_event_file_path
      |> File.read!()
      |> ConcertHallParser.event_selector()
      |> List.first()
  ...
  describe "venue_name/0" do
    test "returns expected value" do
      assert "The Concert Hall" == ConcertHallParser.venue_name()
    end
  end

  describe "event_selector/1" do
    test "returns expected events", %{index_html: index_html} do
      events = ConcertHallParser.event_selector(index_html)
