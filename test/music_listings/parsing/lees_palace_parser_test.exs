defmodule MusicListings.Parsing.LeesPalaceParserTest do
  use ExUnit.Case, async: true

  alias MusicListings.Parsing.LeesPalaceParser
  ...
  setup do
    ...
    event =
      single_event_file_path
      |> File.read!()
      |> LeesPalaceParser.event_selector()
      |> List.first()
  ...
  describe "venue_name/0" do
    test "returns expected value" do
      assert "Lee's Palace" == LeesPalaceParser.venue_name()
    end
  end

  describe "event_selector/1" do
    test "returns expected events", %{index_html: index_html} do
      events = LeesPalaceParser.event_selector(index_html)
