defmodule MusicListings.Parsing.ScotiabankParserTest do
  use ExUnit.Case, async: true

  alias MusicListings.Parsing.Performers
  alias MusicListings.Parsing.Price
  alias MusicListings.Parsing.ScotiabankParser
  ...
  setup do
    ...
    event =
      single_event_file_path
      |> File.read!()
      |> ScotiabankParser.event_selector()
      |> List.first()
  ...
  describe "venue_name/0" do
    test "returns expected value" do
      assert "Scotiabank Arena" == ScotiabankParser.venue_name()
    end
  end

  describe "event_selector/1" do
    test "returns expected events", %{index_html: index_html} do
      events = ScotiabankParser.event_selector(index_html)
