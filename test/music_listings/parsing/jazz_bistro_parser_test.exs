defmodule MusicListings.Parsing.JazzBistroParserTest do
  use ExUnit.Case, async: true

  alias MusicListings.Parsing.JazzBistroParser
  ...
  setup do
    ...
    event =
      single_event_file_path
      |> File.read!()
      |> JazzBistroParser.event_selector()
      |> List.first()
  ...
  describe "venue_name/0" do
    test "returns expected value" do
      assert "Jazz Bistro" == JazzBistroParser.venue_name()
    end
  end

  describe "event_selector/1" do
    test "returns expected events", %{index_html: index_html} do
      events = JazzBistroParser.event_selector(index_html)
