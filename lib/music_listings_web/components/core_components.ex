defmodule MusicListingsWeb.CoreComponents do
  @moduledoc """
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div>
      ...
      <select
        id={@id}
        name={@name}
        class="mt-2 block w-full rounded-md border-0 bg-white/5 py-1.5 text-white shadow-sm ring-1 ring-inset ring-white/10 focus:ring-2 focus:ring-inset focus:ring-emerald-500 sm:text-sm sm:leading-6"
        multiple={@multiple}
        {@rest}
      >
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(MusicListingsWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(MusicListingsWeb.Gettext, "errors", msg, opts)
    end
  end
